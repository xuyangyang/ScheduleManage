...
public class ScheduleDAO {
	...
	public int save(ScheduleVO scheduleVO){
		...
		values.put("scheduleTitle", scheduleVO.getScheduleTtile());
		values.put("scheduleLocation", scheduleVO.getScheduleLoaction());
		...
		values.put("alartimeend", scheduleVO.getAlartimeend());
		db.beginTransaction();
		int scheduleID = -1;
		try{
	...
	//	dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		//System.out.println(scheduleID+"--------------len");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		//Cursor cursor = db.query("schedule", new String[]{"scheduleID","scheduleTitle","scheduleLocation","remindID","scheduleContent","scheduleDate","time","alartime"}, "scheduleID=?", new String[]{String.valueOf(scheduleID)}, null, null, null,null);
		Cursor cursor = db.query("schedule",null, "scheduleID=?", new String[]{String.valueOf(scheduleID)}, null, null, null,null);
		//System.out.println(cursor.getCount()+"--------------len");
		if(cursor.moveToFirst()){
			
			int schID = cursor.getInt(cursor.getColumnIndex("scheduleID"));
			String scheduleTitle = cursor.getString(cursor.getColumnIndex("scheduleTitle"));
			String scheduleLocation = cursor.getString(cursor.getColumnIndex("scheduleLocation"));
			...
			long alartimeend=cursor.getLong(cursor.getColumnIndex("alartimeend"));
			cursor.close();
			db.close();
			return new ScheduleVO(schID,scheduleTitle,scheduleLocation,remindID,scheduleContent,scheduleDate,time,alartime,alartimeend);
		}
		cursor.close();
		db.close();
	...
	public ArrayList<ScheduleVO> getAllSchedule(){
		...
		//dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		Cursor cursor = db.query("schedule", new String[]{"scheduleID","scheduleTitle","scheduleLocation","remindID","scheduleContent","scheduleDate","time","alartime","alartimeend"}, null, null, null, null, "scheduleID desc",null);
		while(cursor.moveToNext()){
			...
			long alartime=cursor.getLong(cursor.getColumnIndex("alartime"));
			long alartimeend=cursor.getLong(cursor.getColumnIndex("alartimeend"));
			ScheduleVO vo = new ScheduleVO(scheduleID,scheduleTitle,scheduleLocation,remindID,scheduleContent,scheduleDate,time,alartime,alartimeend);
			list.add(vo);
		}
		cursor.close();
		db.close();
	...
	public void update(ScheduleVO vo){
		...
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put("scheduleTitle", vo.getScheduleTtile());
		values.put("scheduleLoaction", vo.getScheduleLoaction());
		...
		values.put("alartime", vo.getAlartime());
		values.put("alartimeend", vo.getAlartimeend());
